...
- **Slice D — Bundle (accommodation only)** shipped. New bookings columns `hotel_id`, `hotel_nightly_cents`, `hotel_total_cents` with a check-constraint enforcing all-three-or-none consistency. `/book/[slug]/confirm` gained a "Where to stay" radio group — self-arranged default + one card per partner hotel from Directus (image, name, summary, per-night + total math). Hotel choice URL-synced via `?hotel=<id>`. Summary panel shows a price breakdown (programme + hotel + grand total) that updates live as the user picks. `/api/bookings/create` accepts `hotelId`, validates it belongs to the centre, captures the trusted nightly rate server-side, computes total. `/book/success` and the confirmation email now include the hotel line + grand total. Gear + add-ons remain deferred until partner demand; live availability checking waits for allocation tables.
- **Slice I — Centre admin (lean)** shipped, co-located in guest-site under `/admin/centre/*`. New `centre_admins(user_id, centre_slug)` table with RLS-enforcing helper functions `is_centre_admin_of(slug)` and `is_centre_admin_of_booking_user(user_id)`; new RLS policies let admins SELECT bookings + rider_profiles for centres they administer (own-row policies remain). `bookings` gains a `guest_email` snapshot column so the portal never needs to cross into the auth schema. `/api/me` now returns `adminCentres: string[]`; new `useCurrentUser` exposes it. Route middleware `admin-centre.ts` redirects anon to /login, non-admins to /account. Admin pages: `/admin/centre` (auto-redirects to bookings for single-centre admins), `/admin/centre/bookings` (newest-first table — booking ref, guest, dates, total, status pill, placed-at), `/admin/centre/bookings/[ref]` (read-only detail — guest profile, programme, hotel, system metadata). Andy provisioned as the Karpathos admin. The dedicated `apps/centre-admin` Nuxt scaffold is held in reserve — split when scale or release-cadence demands it. Status transitions and availability editing are future chunks. Lights up Epic 4 stories 4.5 and 4.6.
- **Slice J — Hotel admin (lean)** shipped, co-located alongside Slice I under `/admin/hotel/*`. New `hotel_admins(user_id, hotel_id)` table + helper functions `is_hotel_admin_of(hotel)` and `is_hotel_admin_of_booking_user(user_id)`. RLS policies: hotel admins SELECT only bookings where `bookings.hotel_id` matches a hotel they administer (no-hotel and other-hotel bookings invisible) plus the rider profiles of those guests. `/api/me` now also returns `adminHotels: string[]`. New `admin-hotel.ts` route middleware. Pages: `/admin/hotel` auto-redirect picker, `/admin/hotel/bookings` arrival-sorted table with stay + hotel-revenue columns, `/admin/hotel/bookings/[ref]` stay-first detail page (programme info shown but de-emphasised — hotel staff don't manage that). Provisioning is identical to centre admins via service-role insert. Lights up Epic 5 stories 5.5 (hotel bookings management) and 5.4 (calendar overview, partial — table view only).
- **Slice H — Polish / a11y audit** shipped. Added `@axe-core/playwright` and an end-to-end a11y smoke that visits all eight public pages (home, destinations, karpathos, wing, book picker, login, signup, forgot-password) and fails the build on any critical or serious WCAG 2.1 AA violation. Initial run flagged contrast issues on the Aegean palette, fixed by sweeping: `text-accent-600` → `text-accent-700` (links/eyebrows; #b03625 ≥ 4.5:1 on white), `bg-accent-500/600` → `bg-accent-700` for white-text CTAs (#b03625 + white passes), `text-accent-200/100` → `text-accent-50` for eyebrows on `bg-primary-700`, `text-primary-500` → `text-primary-700` for muted small text. Layout meta gained `theme-color`, app preconnect added for `images.unsplash.com`. Hover-scale animations now wrapped in `motion-safe:` so users with `prefers-reduced-motion` aren't moved. Lighthouse mobile run is the manual next step against the prod URL.
- **Slice K — Activity feed (Facebook page plugin)** shipped. `/journal` page renders ION Karpathos's live Facebook timeline via FB's official Page Plugin iframe — no Meta API, no auth, no scraping, no curation. Source URL is `runtimeConfig.public.facebookPageUrl` (env: `NUXT_PUBLIC_FACEBOOK_PAGE_URL`, currently `https://www.facebook.com/ion.club.karpathos`); empty value renders a "Feed coming soon" message. Includes a fallback link for visitors with FB-blockers. axe e2e suite extended to cover /journal; iframe contents excluded from axe scope (FB's own ARIA isn't ours to fix). The previous deferral of the activity feed is now un-deferred — automated Graph-API ingestion is still later if the plugin proves limiting.
- **Slice K layout v2** — repositioned. `/journal` now leads with **live conditions** (current wind / direction / temp + 6-hour forecast strip), with the FB feed demoted to a sidebar. Conditions backed by Open-Meteo (free, no auth) via a new `defineCachedEventHandler`-based `/api/conditions/[slug]` route with 10-minute SWR cache. Coordinates currently hardcoded per-slug in the route file; wt_centres lat/lng can land later without changing the API surface. Wind speed in knots (`wind_speed_unit=kn`), 8-point compass, WMO weather codes mapped to short labels. A purpose-built `/karpathos/conditions` dashboard (and the `/journal` editorial-feed redesign once we land a real data path with ION) are noted as follow-ups but not yet on the slate. Also moved canonical `<html lang>` + `theme-color` meta from `useHead` to `app.head` in nuxt.config so axe's `html-has-lang` rule passes consistently in dev.
- **Epic 12 — Conditions and wind dashboard (WT-94)** complete. Six stories shipped: WT-95 (centre coords on Directus, replacing the hardcoded const so any future centre lights up automatically), WT-96 (`/api/conditions/[slug]/forecast?days=14` daily aggregates from Open-Meteo, 10-min SWR), WT-97 (`/api/conditions/[slug]/history?from&to&years` archive lookup with computed p25/p75/mean per day, 24h cache), WT-98 (`/[slug]/conditions` dashboard page), WT-99 (year-over-year SVG comparison chart — coral forecast line + shaded historical band + dotted mean line, no chart library, with collapsible data-table fallback), WT-100 (wind-now chip on `/[slug]` hero linking to the conditions page). WT-101 (marine/water-temp data) deferred. Booking flow now deep-links to the conditions page from confirm and success ("See the wind for your dates →") with `?from=&to=` URL params so the historical comparison auto-aligns to the trip dates.
- **2026-04-25 Lighthouse mobile baseline** (commit `45b05eb`, public Vercel prod): `/` 87/100/100/100, `/karpathos` 81/100/100/100, `/karpathos/conditions` 86/100/96/100, `/journal` 91/100/96/100, `/book` 81/100/100/58. Order: performance / accessibility / best-practices / SEO. SEO 58 on `/book` is the intentional `noindex` (booking pages); accessibility is 100 across the public surface (axe e2e suite paid off). Performance straddles the MVP-plan 90-mobile target — `/journal` clears, `/karpathos` and `/book` (both image-heavy) fall short. Future polish: hero-image preload, lazy-load below-fold sections, font-display swap audit.
- **2026-04-25 Performance polish v1** (commit `62c4e32`). Three targeted changes: (a) `<link rel="preload" as="image" fetchpriority="high">` for the centre hero on `/[slug]` via `useHead`; (b) `content-visibility: auto` + `contain-intrinsic-size` on the four below-fold sections (overview / products / hotels / closing CTA) so the browser skips layout + paint until they enter the viewport; (c) first centre card on `/book` and `/destinations` switches `loading="lazy" → "eager"` + `fetchpriority="high"`. Re-measured: `/` 87 → **90**, `/karpathos` 81 → 83, `/destinations` 84, `/book` 81 (flat). Hero preload URL matches the rendered srcset URL exactly so the cache hits — gap to 90 on `/karpathos` is image weight, not first byte. NuxtImg currently emits the same 1600px URL for both `1w` and `2w`, so mobile downloads desktop-sized images. Lifting `/karpathos` over 90 requires real responsive variants — a `@nuxt/image` Vercel-provider config switch — which is a separate change and a bigger swing than today's polish budget. Logged as a future polish item.
